// Package bookingres holds the response bodies for the Hotel Booking APIs.
//
// A hotel order corresponds to one PNR in the Amadeus GDS. It holds one or more
// hotel bookings, each covering one or more rooms at one property.
//
// See bookingreq for why requests and responses are separate packages.
package bookingres
...
type (
	...
	HotelOffer struct {
		...
		RateFamilyEstimated *dto.RateFamilyEstimatedResponse `json:"rateFamilyEstimated,omitempty"`
	...
	Amenity struct {
		...
		Price *dto.AmenityPrice `json:"price,omitempty"`
