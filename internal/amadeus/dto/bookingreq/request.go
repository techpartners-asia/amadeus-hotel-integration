// Package bookingreq holds the request bodies for the Hotel Booking APIs.
//
// It is separate from bookingres because Amadeus uses the same names on the way
// in and on the way out with different shapes - a request Guest carries a
// caller-assigned tid, a response Guest carries an Amadeus-assigned id. Two
// packages keep both faithful to their own schema rather than renaming one to
// accommodate the other.
//
// This is the only place in the SDK where cardholder data crosses the boundary.
package bookingreq
